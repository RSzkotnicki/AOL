...
Sub FindSN(name As String)
'locates, if possible, member "name"
RunmenubyString ("Locate a Member Online")
Timeout 0.3
aol% = FindWindow("AOL Frame25", vbNullString)
MDI% = FindChildByClass(aol%, "MDIClient")
...
'example:  SizeFormToWindow Me, FindAOLsMDI()
...
Let nextchr$ = Mid$(inptxt$, numspc%, 1)
Let nextchrr$ = Mid$(inptxt$, numspc%, 2)
If nextchr$ = "*" Then Let nextchr$ = GetAOLUser()
...
A = ShowWindow(FindAOL(), 5)
End Sub
Function GetListString(Parent, Index, buffer As String)
...
'Msgbox "You have " & CountMailz & " mails"
themail% = FindChildByClass(FindAOLsMDI(), "AOL Child")
thetree% = FindChildByClass(themail%, "_AOL_Tree")
CountMailz = SendMessage(thetree%, LB_GETCOUNT, 0, 0)
End Function

Sub GoToLobby()
'opens a Lobby chat room
If FindChatroom() Then Exit Sub
KW ("pc")
Do: DoEvents
Loop Until FindChatroom()

End Sub


Sub OpenNewMail(which)
...
Sub RunmenubyStringer(stringer As String)
...
'RunmenubyString("&Compose Mail")
'you must put an & before the letter that is
'underlined
Call RunmenubyString(FindAOL(), stringer)
End Sub
...
'listing mail
mailwin% = GetTopWindow(FindAOLsMDI())
...
Sub ClickAOLIcon(tool)
'this clicks on the toolbar icons
'the first one...mailbox...is 0
'compose mail is 1
'channels is 2 etc...
toolbar% = FindChildByClass(FindAOL(), "AOL Toolbar")
...
End Sub
Function GetTopWindow1()
'gets the topmost window
GetTopWindow = GetTopWindow(FindAOLsMDI())
End Function
...
Sub SetBackPre()
Call RunmenubyString(FindAOL(), "Preferences")

Do: DoEvents
prefer% = FindChildByTitle(FindAOLsMDI(), "Preferences")
...
CloseAOLs% = FindChildByTitle(aolmod%, "Close mail after it has been sent")
aolconfirm% = FindChildByTitle(aolmod%, "Confirm mail after it has been sent")
aolOK% = FindChildByTitle(aolmod%, "OK")
If aolOK% <> 0 And CloseAOLs% <> 0 And aolconfirm% <> 0 Then Exit Do
Loop
sendcon% = SendMessage(CloseAOLs%, BM_SETCHECK, 0, 0)
...
'FindFwdWin = GetParent(FindChildByTitle(FindChildByClass(FindAOLsMDI(), "AOL Child"), "Forward"))
'Exit Function
firs% = GetWindow(FindChildByClass(FindAOL(), "MDIClient"), 5)
forw% = FindChildByTitle(firs%, "Forward")
If forw% <> 0 Then GoTo Greed
firs% = GetWindow(FindChildByClass(FindAOL(), "MDIClient"), GW_CHILD)

Do: DoEvents
firss% = GetWindow(FindChildByClass(FindAOL(), "MDIClient"), 5)
...
End Sub
Sub errorpunter(txt As TextBox)
IM txt, "<h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe <h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe <h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe <h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe <h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe <h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe <h1><h3></html>T<h1><h3></html>o<h1><h3></html>a<h1><h3></html>S<i> <b> T</html> <h3> <h1> <h2> <h2> <br> <h1> ToaST says BYe BYe "
...
Call RunmenubyString(FindAOL(), "&About America Online")
Do: DoEvents
...
'chat room.  ex:  If Gettext() = 0 Then user is
'not in a chat room
childs% = FindChatroom()
...
'message of "message"
Call RunmenubyString(FindAOL(), "Send an Instant Message")
...
Call RunmenubyString(FindAOL(), "Keyword...")
...
LastLineChat = LastLine
End Function

Sub MAIl(Person, subject, message)
'opens a blank mail and sends it to "Person" with
'the subject of "subject" and body of "message"
Call RunmenubyString(FindAOL(), "Compose Mail")
...
Button (FindChildByTitle(aolw%, "OK"))
...
'Timeout/waits for "interval" seconds
Current = Timer
...
If aolOK% <> 0 And CloseAOLs% <> 0 And aolconfirm% <> 0 Then Exit Do
Loop
sendcon% = SendMessage(CloseAOLs%, BM_SETCHECK, 1, 0)
