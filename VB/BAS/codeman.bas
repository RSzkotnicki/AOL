...
Declare Function FindChildByClass Lib "VB_AOL32.dll" (ByVal Parent As Long, ByVal sClass As String) As Long
Declare Function FindChildByTitle Lib "VB_AOL32.dll" (ByVal Parent As Long, ByVal sTitle As String) As Long
Declare Function RunMenuByString Lib "VB_AOL32.dll" (ByVal Parent As Long, ByVal Srch As String) As Long
Declare Function FindChatRoom Lib "VB_AOL32.dll" () As Long
Declare Function Chat Lib "VB_AOL32.dll" (ByVal TxtToSend As String) As Long
...
End Sub
Sub AppPlaywav(File)
SoundName$ = (App.Path + File)
   wFlags% = SND_ASYNC Or SND_NODEFAULT
   x% = sndPlaySound(SoundName$, wFlags%)
End Sub
Sub NameEmulator(Lamer, txt)
rom = AOLFindRoom
aoledit% = FindChildByClass(rom, "_AOL_View")
Y$ = Format$(String$(2, Chr$(13)))
AOLSetText aoledit%, Y$ & Lamer & ":" & Chr(9) & txt
End Sub
...
AppActivate "America  Online"
'Turns IM's off
Call AOLInstantMessage("$IM_OFF", "¸.·´¯`·¤Þî^^Þ Þµñ†è®")
If AOLFindRoom() Then
AOLChatSend "¸.·´¯`·¤Þî^^Þ Þµñ†è® IM's OFF"
End If
End Sub
Sub dance(M As Form)
...
End Sub
Sub explode(Form As Form, CFlag As Integer)
...
End Sub
Sub ChangeAOLHostName(AOLPath, NewName)
AOL% = FindWindow("AOL Frame25", 0&)
If AOL% = 0 Then
    MsgBox "Please run AOL so I can detect which version of America Online you are using.", 64, "Run America Online!"
    Exit Sub
End If
If AOLVersion() = 2.5 Then
    x = FileExists(AOLPath & "\tool\chat.aol")
    If x = 0 Then
        MsgBox AOLPath & "\tool\chat.aol could not be found.", 22, "Error!"
        Exit Sub
    End If
    FNum = FreeFile
    Open AOLPath & "\tool\chat.aol" For Binary As FNum
        Seek FNum, 6885
        Put FNum, , NewName
    Close FNum
    Exit Sub
...
If AOLVersion() = 3 Then
    x = FileExists(AOLPath & "\tool\aolchat.aol")
    If x = 0 Then
        MsgBox AOLPath & "\tool\aolchat.aol could not be found.", 22, "Error!"
        Exit Sub
    End If
    FNum = FreeFile
    Open AOLPath & "\tool\aolchat.aol" For Binary As FNum
        Seek FNum, 4747
        FF = NewName & "hello lamer" & NewName
        Put FNum, , FF
    Close FNum
    Exit Sub
End If
...
Percent = r
End Function
Sub editgoto(room, keyword)
Call RunMenuByString(AOLWindow(), "Edit Go To Menu")
AOL% = FindWindow("AOL Frame25", 0&)
Do
aolmod% = FindWindow("_AOL_Modal", "Favorite Places")
firstbox% = FindChildByClass(aolmod%, "_AOL_Edit")
secondbox% = GetWindow(firstbox%, GW_HWNDNEXT)
If firstbox% <> 0 And secondbox% <> 0 Then Exit Do
Loop
A% = SendMessageByString(firstbox%, &HC, 0, room)
A% = SendMessageByString(secondbox%, &HC, 0, keyword)
savechanges% = FindChildByTitle(aolmod%, "Save Changes")
Click savechanges%
Click savechanges%
End Sub
...
AppActivate "America  Online"
'turns IM's on
Call AOLInstantMessage("$IM_ON", "¸.·´¯`·¤Þî^^Þ Þµñ†è®")
If AOLFindRoom() Then
AOLChatSend "¸.·´¯`·¤Þî^^Þ Þµñ†è® IM's ON"
End If
End Sub
...
dd% = SendMessage(icon%, WM_LBUTTONDOWN, 0, 0&)
dd% = SendMessage(icon%, WM_LBUTTONUP, 0, 0&)
End Sub
Sub Click(Button%)
D = SendMessageByNum(Button%, WM_LBUTTONDOWN, 0, 0)
D = SendMessageByNum(Button%, WM_LBUTTONUP, 0, 0)
End Sub
...
End Sub
Sub killwin(hWnd%)
r = SendMessage(hWnd%, WM_CLOSE, 0, 0)
End Sub
...
End Function '
Sub WriteINI(main$, place$, toput$, AppName$)
r% = WritePrivateProfileString(main$, place$, toput$, AppName$)
End Sub
Sub IMstatisCheck()
Dim Person As String
Person = LinkSender.Person.text
Do: DoEvents
bah% = FindChildByTitle(AOL%, "Send Instant Message")
aolmsg% = FindWindow("#32770", "America Online")
If aolmsg <> 0 Then
static1% = FindChildByClass(aolmsg, "Static")
static2% = GetWindow(static1%, 2)
txt1 = getwintext(static2%)
killwin aolmsg%
killwin bah%
killwin aolmsg%
killwin bah%
ElseIf txt1 = "" + Person + " cannot currently receive Instant Messages." Then
Timeout (1)
MsgBox "This Person Has Thier Ims Off...", vbOKOnly, "Error In Sending Link"
Exit Sub
End If
If txt1 = "" + Person + " is not currently signed on." Then
Timeout (1)
MsgBox "This Person Is Not Signed On...", vbOKOnly, "Error Sending Link"
Exit Sub
End If
Loop
End Sub
