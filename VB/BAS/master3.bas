...
Lst.Clear
For Index% = 0 To 23 'This will loop until everyone's added or it will stop at 25
namez$ = String$(256, " ")
...
'Run = FindWindowLike(hWnds(), x%, "*", "_AOL_Edit", Null)
...
Dim SD%, SU%, ZX%
    ZX% = setfocusapi(Btn%)
    SD% = sendmessagebynum(Btn%, WM_LBUTTONDOWN, VK_SPACE, 0&)
    SU% = sendmessagebynum(Btn%, WM_LBUTTONUP, VK_SPACE, 0&)
End Sub
...
Dim aol%, MDI%, TopWin%, ChildWin%, Room%, Find%, buf$ 'Dim Varibles
aol% = findwindow("AOL Frame25", 0&) 'Find The Main Win
MDI% = findchildbyclass(aol%, "MDIClient") 'Find The MDIClient
TopWin% = getwindow(MDI%, GW_CHILD) 'Find The TopMost Win
Do: DoEvents
   ChildWin% = getwindow(TopWin%, GW_CHILD) 'Get Child of TopWin%
    Do: DoEvents
    buf$ = String$(255, 0) 'Set buffer
    X = GetClassName(ChildWin%, buf$, Len(buf$) + 1) 'Get Name of Child
    buf$ = Trim$(buf$) 'Trim Nulls
    If InStr(buf$, "_AOL_Listbox") Then Find% = Find% + 1 'Look For Listbox
    If InStr(buf$, "_AOL_View") Then Find% = Find% + 1 'Look for AOL View
    If InStr(buf$, "_AOL_Edit") Then Find% = Find% + 1 'Look For Edit Box
    If Find% = 3 Then 'If The ChatRoom is Found Then...
   Room% = getparent(ChildWin%) 'Get The Handle of The Chat Win
   findchatroom = Room%: Exit Function 'Set The Function = ChatWin & Exit Func
   End If
   ChildWin% = getwindow(ChildWin%, GW_HWNDNEXT) 'Get Next Win
   Loop While ChildWin% <> 0 'Loop Until All Child Windows have been looked at
Find% = 0 'Reset Find To 0
TopWin% = getwindow(TopWin%, GW_HWNDNEXT) 'Get Next Main Win
Loop While TopWin% <> 0 'Loop Until Every main win has been checked
'Since this function is used a lot, everytime I use, please remind me, I LOVE JESSEE RIGGLE!
End Function
...
Buffer$ = String$(255, 0)
FZ = getfocus()
Debug.Print GetWindowText(FZ, Buffer, 255)
Debug.Print Buffer
...
Call textset(txtbx, "PuNk by NoFx")
...
Function MENUTRIM(Ands) As String
Dim X, a, B
     For X = 1 To Len(Ands)
...
Function runmenubystring(classnameofwindow As String, top_string As String, Sub_String As String) As Integer
     
     'example:
     'activate% = RunMenuByString("AOL Frame25", "Members", "Send an Instant Message")
     
     aol = findwindow(classnameofwindow, 0&)
     a = GetMenu(aol)
     
For X = 0 To GetMenuItemCount%(a)
          Buffer1$ = Space$(255)
          B = GetMenuString(a, X, Buffer1$, 255, MF_BYPOSITION)
          Buffer1$ = Trim$(Buffer1$)
          Buffer1$ = Mid$(Buffer1$, 1, Len(Buffer1$) - 1)
          Buffer1$ = MENUTRIM(Buffer1$)
          If Buffer1$ = top_string Then
                    TopMenuPos = X
                    Exit For
          End If
Next X
     If TopMenuPos = 0 Then
          runmenubystring = False
          Exit Function
     End If
     
     SubMnuhWnd = GetSubMenu(a, TopMenuPos)
     
     For z = 0 To GetMenuItemCount(SubMnuhWnd)
          Buffer2$ = Space$(255)
          c = GetMenuString(SubMnuhWnd, z, Buffer2$, 255, MF_BYPOSITION)
          Buffer2$ = Trim$(Buffer2$)
               Buffer2$ = MENUTRIM(Buffer2$)
               If Buffer2$ = Sub_String Then
                   
                    ID = GetMenuItemID(SubMnuhWnd, z)
Dim lParam As Long
Const MF_BYCOMMAND = &H0
lParam = CLng(0) * &H10000 Or MF_BYCOMMAND
i = sendmessagebynum(findwindow(classnameofwindow, 0&), WM_COMMAND, ID, 0&)
                   
                    runmenubystring = True
                    Exit Function
               End If
     Next
...
j% = SetWindowPos(F.hWnd, -1, 0, 0, 0, 0, &H50)
...
Function WriteToINI(iniSection As String, iniParam As String, theData As String, iniFileName As String)
    WriteToINI = WritePrivateProfileString(iniSection, iniParam, theData, iniFileName)
End Function
